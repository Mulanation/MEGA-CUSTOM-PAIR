...
router.get('/', async (req, res) => {
  try {
    ...
    if (!sessionId || !sessionId.startsWith('SUBZERO-MD;;;')) {
      return res.status(400).json({
        valid: false,
        message: "Invalid session ID format. It should start with 'SUBZERO-MD;;;'."
      });
    }

    // Extract the file identifier
    const fileId = sessionId.split('SUBZERO-MD;;;')[1];
    if (!fileId) {
      return res.status(400).json({
        valid: false,
